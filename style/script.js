...
yesBtn.addEventListener('click', () => {
  ...
  nextBtn.textContent = 'Tiếp nhé ❤️';
  nextBtn.className = 'btn btn-next';

  // style nhanh (có thể đưa sang CSS nếu muốn)
  nextBtn.style.marginTop = '20px';
  nextBtn.style.padding = '12px 28px';
  nextBtn.style.fontSize = '1.5rem';
  nextBtn.style.borderRadius = '30px';
  nextBtn.style.border = "2px solid #bdbdbd";   // viền xám nhạt
nextBtn.style.boxSizing = "border-box";    // để viền không làm nút to thêm
  nextBtn.style.cursor = 'pointer';
  nextBtn.style.background = 'linear-gradient(135deg, #ff7eb3, #ff758c)';
  nextBtn.style.color = '#fff';

  nextBtn.addEventListener('click', () => {
    window.location.href = "./love/index.html";// 🔁 ĐỔI LINK Ở ĐÂY
  });

  question.after(nextBtn);
});
